...
export function StudentsPage({ role }: { role: 'admin' | 'teacher' }) {
  ...
  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    return students.filter((s) => (classFilter === 'all' || s.class_name === classFilter) && (!q || `${s.full_name} ${s.nis || ''} ${s.nisn || ''}`.toLowerCase().includes(q)))
...
function StudentModal({ student, parents, classes, onClose, onDone }: { student: Student | null; parents: Account[]; classes: SchoolClass[]; onClose: () => void; onDone: () => void }) {
  const [form, setForm] = useState({ full_name: student?.full_name || '', nis: student?.nis || '', nisn: student?.nisn || '', gender: student?.gender || '', birth_place: student?.birth_place || '', birth_date: student?.birth_date || '', class_name: student?.class_name || '', academic_year: student?.academic_year || classes[0]?.academic_year || '2026/2027', active: student?.is_active ?? true, guardian: '' })
  const [busy, setBusy] = useState(false)
  const [errorText, setErrorText] = useState('')
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setErrorText('')
    const payload = { full_name: form.full_name.trim(), nis: form.nis.trim() || null, nisn: form.nisn.trim() || null, gender: form.gender || null, birth_place: form.birth_place.trim() || null, birth_date: form.birth_date || null, class_name: form.class_name || null, academic_year: form.academic_year.trim() || null, is_active: form.active }
