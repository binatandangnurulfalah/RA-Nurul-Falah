...
export function ReportsPage({ role }: { role: AppRole }) {
  ...
  const filtered = useMemo(() => { const q = search.trim().toLowerCase(); return rows.filter((row) => { const student = studentMap.get(row.student_id); return (role !== 'parent' || !childSelection.selectedChildId || row.student_id === childSelection.selectedChildId) && (!q || `${student?.full_name || ''} ${student?.class_name || ''} ${row.academic_year}`.toLowerCase().includes(q)) }) }, [rows, search, studentMap, role, childSelection.selectedChildId])
...
export function PaymentsPage({ role }: { role: 'admin' | 'parent' }) {
  ...
  useEffect(() => { void load() }, [])
  const studentMap = useMemo(() => new Map(students.map((student) => [student.id, student])), [students]); const visibleRows = role === 'parent' && childSelection.selectedChildId ? rows.filter((row) => row.student_id === childSelection.selectedChildId) : rows; const filtered = useMemo(() => { const q = search.trim().toLowerCase(); return visibleRows.filter((row) => { const student = studentMap.get(row.student_id); return !q || `${student?.full_name || ''} ${row.payment_type} ${row.period_label || ''}`.toLowerCase().includes(q) }) }, [visibleRows, search, studentMap])
