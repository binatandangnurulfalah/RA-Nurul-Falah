...
export function ReportsPage({ role }: { role: AppRole }) {
  ...
  const filtered = useMemo(() => { const q = debouncedSearch.trim().toLowerCase(); return rows.filter((row) => { const student = studentMap.get(row.student_id); return (role !== 'parent' || !childSelection.selectedChildId || row.student_id === childSelection.selectedChildId) && (!q || `${student?.full_name || ''} ${student?.class_name || ''} ${row.academic_year}`.toLowerCase().includes(q)) }) }, [rows, debouncedSearch, studentMap, role, childSelection.selectedChildId])
  const paged = usePaginatedItems(filtered, `${debouncedSearch}:${childSelection.selectedChildId}`)
...
function ReportViewer({ value, student, onClose }: { value: ReportCard; student?: StudentLite; onClose: () => void }) {
  ...
  return <Modal title={`Rapor ${student?.full_name || 'Murid'}`} onClose={onClose} wide><div className="v5-print-document"><div className="report-view-head"><span className="v2-badge blue">Semester {value.semester}</span><span>{student?.class_name || 'RA Nurul Falah'}</span><span>{value.academic_year}</span></div><div className="report-view-sections">{items.map(([label, text]) => <section key={label}><small>{label}</small><p>{text || 'Belum ada catatan.'}</p></section>)}</div></div><button className="v2-primary v5-print-button" onClick={() => window.print()}><Printer size={17} /> Cetak / Simpan PDF</button></Modal>
}

export function PaymentsPage({ role }: { role: 'admin' | 'parent' }) {
  const canManage = role === 'admin'
  const childSelection = useChildSelection()
  const [students, setStudents] = useState<StudentLite[]>([]); const [rows, setRows] = useState<Payment[]>([]); const [loading, setLoading] = useState(true); const [search, setSearch] = useState(''); const [editing, setEditing] = useState<Payment | 'new' | null>(null); const [deleting, setDeleting] = useState<Payment | null>(null); const [receipt, setReceipt] = useState<Payment | null>(null); const [message, setMessage] = useState<Message | null>(null)
  const debouncedSearch = useDebouncedValue(search)
  ...
  useEffect(() => { void load() }, [])
  const studentMap = useMemo(() => new Map(students.map((student) => [student.id, student])), [students]); const visibleRows = role === 'parent' && childSelection.selectedChildId ? rows.filter((row) => row.student_id === childSelection.selectedChildId) : rows; const filtered = useMemo(() => { const q = debouncedSearch.trim().toLowerCase(); return visibleRows.filter((row) => { const student = studentMap.get(row.student_id); return !q || `${student?.full_name || ''} ${row.payment_type} ${row.period_label || ''}`.toLowerCase().includes(q) }) }, [visibleRows, debouncedSearch, studentMap])
  const paged = usePaginatedItems(filtered, `${debouncedSearch}:${childSelection.selectedChildId}`)
  const total = visibleRows.reduce((sum, row) => sum + Number(row.amount), 0); const paid = visibleRows.reduce((sum, row) => sum + Math.min(Number(row.amount), Number(row.paid_amount)), 0); const outstanding = visibleRows.filter((row) => row.status !== 'waived').reduce((sum, row) => sum + Math.max(0, Number(row.amount) - Number(row.paid_amount)), 0)
  ...
  return <div className="v2-stack"><PageTitle eyebrow="KEUANGAN SEKOLAH" title="Pembayaran" text={canManage ? 'Kelola tagihan, pembayaran, jatuh tempo dan status pembayaran murid.' : 'Pantau tagihan dan pembayaran anak yang terhubung.'} action={canManage ? <button className="v2-primary" onClick={() => setEditing('new')}><Plus size={17} /> Tambah Tagihan</button> : undefined} />{message && <Notice {...message} />}<div className="v2-stat-grid three"><MoneyStat label="Total Tagihan" value={total} tone="gold" /><MoneyStat label="Sudah Dibayar" value={paid} tone="green" /><MoneyStat label="Sisa Tagihan" value={outstanding} tone="blue" /></div><section className="v2-panel"><div className="v2-toolbar"><label><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari murid, jenis atau periode pembayaran..." /></label></div>{loading ? <SkeletonRows /> : filtered.length ? <><div className="payment-list">{paged.items.map((row) => { const student = studentMap.get(row.student_id); return <article key={row.id}><span className="payment-icon"><BadgeDollarSign /></span><div className="grow"><div className="school-meta"><span className={`v2-badge ${paymentTone(row.status)}`}>{paymentLabel(row.status)}</span>{row.period_label && <span>{row.period_label}</span>}{row.due_date && <span>Jatuh tempo {dateText(row.due_date)}</span>}</div><h3>{row.payment_type}</h3><p>{student?.full_name || 'Murid'} · {student?.class_name || 'Belum ada kelompok'}</p><small>{currency(Number(row.paid_amount))} dari {currency(Number(row.amount))} dibayar</small></div><strong className="payment-balance">{row.status === 'waived' ? 'Dibebaskan' : currency(Math.max(0, Number(row.amount) - Number(row.paid_amount)))}</strong><ActionMenu label={`Aksi pembayaran ${row.payment_type}`} items={[{ label: 'Lihat kuitansi', icon: ReceiptText, onSelect: () => setReceipt(row) }, ...(canManage ? [{ label: 'Edit pembayaran', icon: Edit3, onSelect: () => setEditing(row) }, { label: 'Hapus pembayaran', icon: Trash2, danger: true, onSelect: () => setDeleting(row) }] : [])]} /></article>})}</div><PaginationControls page={paged.page} total={paged.total} onPage={paged.setPage} /></> : <EmptyCard text="Belum ada data pembayaran." />}</section>{editing && canManage && <PaymentModal value={editing === 'new' ? null : editing} students={students} onClose={() => setEditing(null)} onDone={async () => { setEditing(null); setMessage({ tone: 'success', text: 'Data pembayaran berhasil disimpan.' }); await load() }} />}{receipt && <PaymentReceipt payment={receipt} student={studentMap.get(receipt.student_id)} onClose={() => setReceipt(null)} />}{deleting && <Confirm title="Hapus pembayaran?" text={`${deleting.payment_type} akan dihapus dari riwayat pembayaran.`} onClose={() => setDeleting(null)} onConfirm={() => void remove()} />}</div>
}

function PaymentReceipt({ payment, student, onClose }: { payment: Payment; student?: StudentLite; onClose: () => void }) {
  return <Modal title="Kuitansi Pembayaran" onClose={onClose}><div className="v5-print-document v5-receipt"><header><strong>RA NURUL FALAH</strong><small>Bukti pembayaran sekolah</small></header><dl><div><dt>Nama murid</dt><dd>{student?.full_name || 'Murid'}</dd></div><div><dt>Jenis pembayaran</dt><dd>{payment.payment_type}</dd></div><div><dt>Periode</dt><dd>{payment.period_label || '—'}</dd></div><div><dt>Nominal dibayar</dt><dd>{currency(Number(payment.paid_amount))}</dd></div><div><dt>Status</dt><dd>{paymentLabel(payment.status)}</dd></div><div><dt>Tanggal</dt><dd>{payment.paid_at ? dateText(payment.paid_at.slice(0, 10)) : 'Belum dibayar'}</dd></div></dl><p>Dicetak dari Sistem Informasi RA Nurul Falah.</p></div><button className="v2-primary v5-print-button" onClick={() => window.print()}><Printer size={17} /> Cetak / Simpan PDF</button></Modal>
}
...
export function DocumentsPage({ role }: { role: AppRole }) {
  ...
  useEffect(() => { void load() }, [])
  const filtered = useMemo(() => { const q = debouncedSearch.trim().toLowerCase(); return rows.filter((row) => !q || `${row.title} ${row.category} ${row.document_number || ''} ${row.recipient || ''}`.toLowerCase().includes(q)) }, [rows, debouncedSearch]); const paged = usePaginatedItems(filtered, debouncedSearch); const remove = async () => { if (!deleting || !canManage) return; const { error } = await supabase.from('school_documents').delete().eq('id', deleting.id); if (error) { setMessage({ tone: 'error', text: error.message }); return }; setDeleting(null); setMessage({ tone: 'success', text: 'Dokumen berhasil dihapus.' }); await load() }
  const openDocument = async (row: SchoolDocument) => { if (!row.file_url) return; if (/^https?:\/\//.test(row.file_url)) { window.open(row.file_url, '_blank', 'noopener,noreferrer'); return }; const { data, error } = await supabase.storage.from('school-documents').createSignedUrl(row.file_url, 300); if (error || !data?.signedUrl) { setMessage({ tone: 'error', text: 'Dokumen gagal dibuka. Silakan coba lagi.' }); return }; window.open(data.signedUrl, '_blank', 'noopener,noreferrer') }
  return <div className="v2-stack"><PageTitle eyebrow="ARSIP SEKOLAH" title="Dokumen & Surat" text={canManage ? 'Kelola surat, arsip dan unggah dokumen resmi sekolah.' : 'Dokumen resmi yang dibagikan kepada akun Anda.'} action={canManage ? <button className="v2-primary" onClick={() => setEditing('new')}><Plus size={17} /> Tambah Dokumen</button> : undefined} />{message && <Notice {...message} />}<section className="v2-panel"><div className="v2-toolbar"><label><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari judul, kategori, nomor surat..." /></label></div>{loading ? <SkeletonRows /> : filtered.length ? <><div className="document-grid">{paged.items.map((row) => <article key={row.id}><span className="document-icon"><FileText /></span><div className="grow"><div className="school-meta"><span className={`v2-badge ${row.is_published ? 'green' : 'gray'}`}>{row.is_published ? 'Terbit' : 'Draft'}</span><span>{audienceLabel(row.audience)}</span>{row.document_date && <span>{dateText(row.document_date)}</span>}</div><h3>{row.title}</h3><p>{row.category}{row.document_number ? ` · ${row.document_number}` : ''}</p><small>{row.description || row.recipient || 'Tidak ada keterangan tambahan.'}</small></div><ActionMenu label={`Aksi dokumen ${row.title}`} items={[...(row.file_url ? [{ label: 'Buka dokumen', icon: ExternalLink, onSelect: () => void openDocument(row) }] : []), ...(canManage ? [{ label: 'Edit dokumen', icon: Edit3, onSelect: () => setEditing(row) }, { label: 'Hapus dokumen', icon: Trash2, danger: true, onSelect: () => setDeleting(row) }] : [])]} /></article>)}</div><PaginationControls page={paged.page} total={paged.total} onPage={paged.setPage} /></> : <EmptyCard text="Belum ada dokumen yang tersedia." />}</section>{editing && canManage && <DocumentModal value={editing === 'new' ? null : editing} onClose={() => setEditing(null)} onDone={async () => { setEditing(null); setMessage({ tone: 'success', text: 'Dokumen berhasil disimpan.' }); await load() }} />}{deleting && <Confirm title="Hapus dokumen?" text={deleting.title} onClose={() => setDeleting(null)} onConfirm={() => void remove()} />}</div>
}
...
function DocumentModal({ value, onClose, onDone }: { value: SchoolDocument | null; onClose: () => void; onDone: () => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setErrorText(''); let filePath = form.url.trim() || null; if (file) { const safeName = file.name.toLowerCase().replace(/[^a-z0-9._-]+/g, '-'); filePath = `${new Date().getFullYear()}/${crypto.randomUUID()}-${safeName}`; const upload = await supabase.storage.from('school-documents').upload(filePath, file, { upsert: false, contentType: file.type }); if (upload.error) { setBusy(false); setErrorText(upload.error.message); return } }; const payload = { title: form.title.trim(), category: form.category.trim() || 'Umum', document_number: form.number.trim() || null, document_date: form.date || null, recipient: form.recipient.trim() || null, description: form.description.trim() || null, file_url: filePath, audience: form.audience, is_published: form.published }; const result = value ? await supabase.from('school_documents').update(payload).eq('id', value.id) : await supabase.from('school_documents').insert(payload); setBusy(false); if (result.error) { if (filePath && file) await supabase.storage.from('school-documents').remove([filePath]); setErrorText(result.error.message); return }; onDone() }
  return <Modal title={value ? 'Edit Dokumen' : 'Tambah Dokumen'} onClose={onClose} wide><form className="v2-form v2-form-grid" onSubmit={submit}><label className="full">Judul dokumen<input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label>Kategori<input required value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Surat Edaran / Formulir / Arsip" /></label><label>Nomor surat<input value={form.number} onChange={(e) => setForm({ ...form, number: e.target.value })} /></label><label>Tanggal dokumen<input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></label><label>Penerima / tujuan<input value={form.recipient} onChange={(e) => setForm({ ...form, recipient: e.target.value })} /></label><label>Ditampilkan kepada<select value={form.audience} onChange={(e) => setForm({ ...form, audience: e.target.value as SchoolDocument['audience'] })}><option value="all">Guru & Wali</option><option value="teacher">Guru saja</option><option value="parent">Wali saja</option><option value="admin">Admin saja</option></select></label><label className="v2-toggle"><input type="checkbox" checked={form.published} onChange={(e) => setForm({ ...form, published: e.target.checked })} /><span>Publikasikan</span></label><label className="full v5-file-field"><Upload size={17} /> Unggah file<input type="file" accept=".pdf,.jpg,.jpeg,.png,.docx" onChange={(event) => setFile(event.target.files?.[0] || null)} /><small>{file ? file.name : value?.file_url ? 'Gunakan file tersimpan atau pilih pengganti.' : 'PDF, JPG, PNG, atau DOCX · maksimal 10 MB'}</small></label><label className="full">Atau tautan eksternal<input type="url" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} placeholder="https://... (opsional)" /></label><label className="full">Deskripsi<textarea rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label>{errorText && <p className="v2-field-error full">{errorText}</p>}<div className="v2-form-actions full"><button type="button" className="v2-secondary" onClick={onClose}>Batal</button><button className="v2-primary" disabled={busy}>{busy ? 'Mengunggah & menyimpan...' : 'Simpan Dokumen'}</button></div></form></Modal>
}

function Modal({ title, onClose, wide = false, children }: { title: string; onClose: () => void; wide?: boolean; children: ReactNode }) { return <Dialog title={title} onClose={onClose} wide={wide}>{children}</Dialog> }
