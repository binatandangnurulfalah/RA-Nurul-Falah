...
export default function SchedulePage({ canManage }: { canManage: boolean }) {
  ...
  return <div className="v2-stack">
    <PageTitle eyebrow="AGENDA BELAJAR" title="Jadwal Mingguan" text={canManage ? 'Tambah, edit, dan hapus jadwal kegiatan belajar.' : 'Jadwal kegiatan belajar anak sesuai kelompoknya.'} action={canManage ? <button className="v2-primary" onClick={() => setEditing('new')}><Plus size={17} /> Tambah Jadwal</button> : undefined} />
...
function ScheduleModal({ value, classes, defaultDay, onClose, onDone }: { value: Schedule | null; classes: SchoolClass[]; defaultDay: number; onClose: () => void; onDone: () => void }) {
  ...
  return <Dialog title={value ? 'Edit Jadwal' : 'Tambah Jadwal'} eyebrow="FORMULIR" onClose={onClose} wide><form className="v2-form v2-form-grid" onSubmit={submit}><label>Kelompok<select required value={form.class_name} onChange={(event) => setForm({ ...form, class_name: event.target.value })}>{classes.map((schoolClass) => <option value={schoolClass.name} key={schoolClass.id}>{schoolClass.name}</option>)}</select></label><label>Hari<select value={form.day} onChange={(event) => setForm({ ...form, day: Number(event.target.value) })}>{['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat'].map((label, index) => <option value={index + 1} key={label}>{label}</option>)}</select></label><label>Jam mulai<input required type="time" value={form.start} onChange={(event) => setForm({ ...form, start: event.target.value })} /></label><label>Jam selesai<input required type="time" value={form.end} onChange={(event) => setForm({ ...form, end: event.target.value })} /></label><label className="full">Kegiatan<input required value={form.activity} onChange={(event) => setForm({ ...form, activity: event.target.value })} /></label><label>Guru<input value={form.teacher} onChange={(event) => setForm({ ...form, teacher: event.target.value })} /></label><label>Tahun ajaran<input required value={form.academic_year} onChange={(event) => setForm({ ...form, academic_year: event.target.value })} /></label>{errorText && <p className="v2-field-error full">{errorText}</p>}<div className="v2-form-actions full"><button type="button" className="v2-secondary" onClick={onClose}>Batal</button><button className="v2-primary" disabled={busy}><Save size={17} /> Simpan Jadwal</button></div></form></Dialog>
}
