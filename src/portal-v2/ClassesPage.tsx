...
export function ClassesPage() {
  ...
  const load = async () => {
    setLoading(true)
    const [classResult, studentResult, teacherResult, yearResult] = await Promise.all([
      supabase
        .from('school_classes')
    ...
    if (classResult.error || studentResult.error || teacherResult.error || yearResult.error) {
      setMessage({
        tone: 'error',
        text: classResult.error?.message || studentResult.error?.message || teacherResult.error?.message || yearResult.error?.message || 'Data akademik gagal dimuat.',
      })
    }
  ...
  return <div className="v2-stack">
    <PageTitle
      ...
      text="Kelola kelompok belajar, penugasan Guru, dan tahun ajaran resmi sekolah."
...
function ClassModal({ value, teachers, academicYears, onClose, onDone }: {
