...
import { ChildrenPage, DashboardPage, ProfilePage, SettingsPage } from './portal-v2/PortalPages'
...
export default function RolePortalV2({ profile }: { profile: UserProfile }) {
...
function PageRouter({ role, page, profile, setProfile, go }: { role: AppRole; page: string; profile: UserProfile; setProfile: (profile: UserProfile) => void; go: (page: string) => void }) {
  ...
  if (page === 'profile') return <ProfilePage profile={profile} onProfileChange={setProfile} />
...
function mobileLabel(label: string) { if (label === 'Manajemen Akun') return 'Akun'; if (label === 'Data Absen') return 'Absen'; if (label === 'Data Murid') return 'Murid'; if (label === 'Data Anak') return 'Anak'; if (label === 'Scan Absensi') return 'Scan'; return label.split(' ')[0] }
